...
while (norm(x-P(x-gradient)) > eps)
  
  Eye = eye(length(hessian),length(hessian));
  hessian(:,A) = Eye(:,A);
  hessian(A,:) = Eye(A,:);
      
  [d] = PrecCGSolver (hessian, -gradient, 1.0e-6);
  if (gradient'*d >= 0)
    d = -gradient;
  endif
  
  [t] = projectedBacktrackingSearch(f, P, x, d, sigma, true);
