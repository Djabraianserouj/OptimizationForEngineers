function [A_Value, A_Gradient,A_Hessian] = augmentedLagrangianObjective(f, h, x, alpha, gamma)
  ...
  #COMPLETE THE CODE
    A_Value = fvalue + alpha*hvalue + 0.5*gamma*hvalue^2
    A_Gradient = fgradient + alpha*hgradient + 0.5*gamma*2*hvalue*hgradient
    A_Hessian = fhessian + alpha*hhessian + 0.5*gamma*(2*hvalue*hhessian+2*hgradient*hgradient')
